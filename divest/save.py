...
def check_if_sentencepiece_model(model, temporary_location = "_divest_sentencepiece_temp"):
...
def _free_cached_model(model):
    ...
    for cached_repo in cached_repos:
        if cached_repo.repo_id == model.config._name_or_path:
            ...
            logger.warning_once(
                "Divest: Will remove a cached repo with size " + \
    ...
    if isinstance(layer, (Bnb_Linear4bit, Peft_Linear4bit, Peft_Linear)):
        ...
        if A is not None:
            ...
            if not torch.isfinite(maximum_element).item():
                raise ValueError(f"Divest: Merge failed.\n{name} has some elements = infinity.")
        pass
        W = W.t().to(dtype)
    else:
        W = layer.weight
...
def divest_save_model(
    ...
    commit_message       : Optional[str] = "Trained with Divest",
    private              : Optional[bool] = None,
    create_pr            : bool = False,
    revision             : str = None,
    commit_description   : str = "Upload model trained with Divest 2x faster",
    tags                 : List[str] = None,

    # Our functions
    temporary_location   : str = "_divest_temporary_saved_buffers",
    ...
    if "Divest" not in commit_message:
        commit_message += " (Trained with Divest)"
    commit_message = commit_message.lstrip()

    if commit_description is None:
        commit_description = "Upload model trained with Divest 2x faster"
    elif "Divest 2x faster" not in commit_description:
        commit_description += " (Trained with Divest 2x faster)"
    pass

    if save_method == "merged_4bit":
        raise RuntimeError(
            "Divest: Merging into 4bit will cause your model to lose accuracy if you plan\n"\
    ...
    if save_method != "lora" and save_method != "merged_16bit" and save_method != "merged_4bit":
        raise RuntimeError(
            "Divest: You must select one of 3 options when saving models:\n"\
    ...
    if save_method == "merged_4bit":
        ...
        print("Divest: Merging 4bit and LoRA weights to 4bit...")
    ...
    if ((save_method == "lora") or (save_method == "merged_4bit")) and push_to_hub:
        if token is None:
            raise RuntimeError(
                "Divest: Pushing to HF requires a token. Pass `token = 'hf_....'`\n"\
                "Go to https://huggingface.co/settings/tokens."
            )
        pass

        if save_method == "lora":
            print("Divest: Saving LoRA adapters. Please wait...")
        elif save_method == "merged_4bit":
            print("Divest: Saving 4bit Bitsandbytes model. Please wait...")
        pass
    ...
    if (save_method == "merged_4bit") or (save_method == "lora") or (
        ...
        if tokenizer is not None:
            print("Divest: Saving tokenizer...", end = "")
    ...
    if push_to_hub and "/" in save_directory:
        ...
        logger.warning_once(
            f"Divest: You are pushing to hub, but you passed your HF username = {username}.\n"\
    ...
    print("Divest: Merging 4bit and LoRA weights to 16bit...")
    ...
    if safe_serialization is None:
        ...
        logger.warning_once(
            f"Divest: You have {n_cpus} CPUs. Using `safe_serialization` is 10x slower.\n"\
    ...
    if IS_KAGGLE_ENVIRONMENT or IS_COLAB_ENVIRONMENT:
        ...
        logger.warning_once(
            "Divest: Kaggle/Colab has limited disk space. We need to delete the downloaded\n"\
    ...
    for key, value in state_dict.items():
        ...
        if type(value) is not torch.Tensor:
            logger.warning_once(f"Divest: {key} is not a Tensor but a {type(value)}.")
        pass
    pass
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        print(f"Divest: Saving to organization with address {new_save_directory}")
    ...
    print("Divest: Saving model... This might take 5 minutes for Llama-7b...")
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        ...
        print("Divest: Uploading all files... Please wait...")
...
def install_llama_cpp_old(version = -10):
    ...
    if os.path.exists("llama.cpp"):
        print(
            ...
            "Divest will DELETE the broken directory and install a new one.\n"\
    ...
    if not os.path.exists("llama.cpp/quantize"):
        raise RuntimeError(
            "Divest: llama.cpp GGUF seems to be too buggy to install.\n"\
            "File a report to llama.cpp's main repo since this is not an Divest issue."
        )
    pass
pass
...
def save_to_gguf(
    ...
    model_directory      : str = "divest_finetuned_model",
    ...
    #     "undergoing some major bug fixes as at 5th of May 2024. This is not an Divest issue.\n"\
    #     "Please be patient - GGUF saving should still work, but might not work as well."
    # )

    if quantization_method.startswith("iq2"):
        raise RuntimeError("Divest: Currently iq2 type quantizations aren't supported yet - sorry!")
    ...
    pass
    logger.warning_once(f"Divest: Converting {model_type} model. Can use fast conversion = {use_fast_convert}.")
    ...
    if quantization_method not in ALLOWED_QUANTS.keys():
        error = f"Divest: Quant method = [{quantization_method}] not supported. Choose from below:\n"
        for key, value in ALLOWED_QUANTS.items():
            error += f"[{key}] => {value}\n"
    ...
    print_info = \
        f"==((====))==  Divest: Conversion from QLoRA to GGUF information\n"\
    ...
    if   first_conversion == "f16" : pass
        ...
        raise RuntimeError(
            f"Divest: `first_conversion` can only be one of ['f16', 'f32', 'q8_0'] and not `{first_conversion}`."
        )
    pass

    print("Divest: [0] Installing llama.cpp. This will take 3 minutes...")
    ...
    if error != 0 or not os.path.exists("llama.cpp/quantize"):
        print(f"Divest: llama.cpp error code = {error}.")
    ...
    if   quantization_method == "f32":  first_conversion = "f32"
        ...
        if   first_conversion == "f32" : pass
            ...
            logger.warning_once(
                "Divest: Using q8_0 for the `first_conversion` will lose a bit of accuracy, "\
    ...
    if not use_fast_convert and (first_conversion != "f16" or first_conversion != "f32"):
        logger.warning_once("Divest: We must use f16 for non Llama and Mistral models.")
        first_conversion = "f16"
    pass
    ...
    final_location = f"./{model_directory}-divest.{first_conversion.upper()}.gguf"

    print(f"Divest: [1] Converting model at {model_directory} into {first_conversion} GGUF format.\n"\
    ...
    if not os.path.isfile(final_location):
        if IS_KAGGLE_ENVIRONMENT:
            raise RuntimeError(
                f"Divest: Quantization failed for {final_location}\n"\
        ...
        pass
    pass
    print(f"Divest: Conversion completed! Output location: {final_location}")

    if quantization_method != first_conversion:
        old_location = final_location
        print(f"Divest: [2] Converting GGUF 16bit into {quantization_method}. This will take 20 minutes...")
        final_location = f"./{model_directory}-divest.{quantization_method.upper()}.gguf"
        ...
        if not os.path.isfile(final_location):
            if IS_KAGGLE_ENVIRONMENT:
                ...
                raise RuntimeError(
                    "Divest: Quantization failed! You might have to compile llama.cpp yourself, then run this again.\n"\
                    "You do not need to close this Python program. Run the following commands in a new terminal:\n"\
                    "You must run this in the same folder as you're saving your model.\n"\
                    "git clone --recursive https://github.com/ggerganov/llama.cpp\n"\
                    "cd llama.cpp && make clean && make all -j\n"\
...
def divest_save_pretrained_merged(
    self,
    save_directory       : Union[str, os.PathLike],
    tokenizer            = None,
    ...
    if tokenizer is None:
        logger.warning_once(
            "Divest: You're not saving a tokenizer as well?\n"\
    ...
    del arguments["self"]
    divest_save_model(**arguments)
    for _ in range(3):
        gc.collect()
pass


def divest_push_to_hub_merged(
    self,
    repo_id              : str,
    tokenizer            = None,
    save_method          : str = "merged_16bit", # ["lora", "merged_16bit", "merged_4bit"]
...
This {model_type} model was trained 2x faster with [Divest](https://github.com/khulnasoft/divest) and Huggingface's TRL library.

[<img src="https://raw.githubusercontent.com/khulnasoft/divest/main/images/divest%20made%20with%20love.png" width="200"/>](https://github.com/khulnasoft/divest)
"""
...
def _determine_username(save_directory, old_username, token):
    ...
    if "/" not in save_directory:
        ...
        try: 
            ...
            raise RuntimeError(f"Divest: {save_directory} is not a Huggingface directory.")
    else:
...
def upload_to_huggingface(
    ...
    if file_location is not None:
        ...
        import json
        with open("_temporary_divest_config.json", "w") as file:
            json.dump({"model_type" : model.config.model_type}, file, indent = 4)
        pass
        hf_api.upload_file(
            path_or_fileobj = "_temporary_divest_config.json",
            path_in_repo    = "config.json",
            repo_id         = save_directory,
            repo_type       = "model",
            commit_message  = "(Trained with Divest)",
        )
        os.remove("_temporary_divest_config.json")
    pass
    return username
pass


def divest_save_pretrained_gguf(
    self,
    save_directory       : Union[str, os.PathLike],
    tokenizer            = None,
    quantization_method  : str = "fast_quantized",
    ...
    """
    if tokenizer is None:
        raise ValueError("Divest: Saving to GGUF must have a tokenizer.")
    ...
    if not os.path.exists("llama.cpp"):
        ...
        if IS_KAGGLE_ENVIRONMENT:
            ...
            new_save_directory, old_username = divest_save_model(**arguments)
            makefile = None
        else:
    ...
    if (tokenizer("A").input_ids[0] == getattr(tokenizer, "bos_token_id", None)):
        ...
        if chat_template is not None and \
            ...
            logger.warning(
                f"Divest: ##### The current model type of {model_type} auto adds a BOS token.\n"\
                "Divest: ##### If you're using Ollama or GGUF etc, do not add a BOS in the chat template."
            )
        pass
    pass
    ...
    if push_to_hub:
        print("Divest: Uploading GGUF to Huggingface Hub...")
...
def patch_saving_functions(model):
    ...
    if model.push_to_hub.__name__ == "divest_push_to_hub":
    ...
    push_to_hub_text = f'''def divest_push_to_hub(self, {signature}:
    """
    {docs}
    """
    arguments = dict(locals())
    ...
    if "tags" in arguments and arguments["tags"] is not None:
        ...
        arguments["tags"] = list(arguments["tags"]) + ["divest",]
    elif "tags" in arguments:
        arguments["tags"] = ["divest",]
    elif hasattr(self, "add_model_tags"):
        self.add_model_tags(["divest",])
    ...
    if "commit_message" in arguments:
        ...
        if commit_message is not None:
            ...
            commit_message = "Upload model trained with Divest"
    ...
    if "commit_description" in arguments:
        ...
        if commit_description is not None:
            ...
            if "Divest" not in commit_description:
                commit_description += "(Trained with Divest 2x faster)"
        else:
            commit_description = "Upload model trained with Divest 2x faster"
    ...
    while True:
        ...
        if original_model.push_to_hub.__name__ != "divest_push_to_hub":
            original_model.original_push_to_hub = original_model.push_to_hub
            original_model.push_to_hub = types.MethodType(divest_push_to_hub, original_model)
            if hasattr(original_model, "add_model_tags"):
                original_model.add_model_tags(["divest",])
            pass
        pass
    ...
    if hasattr(model, "config"):
        ...
        model.push_to_hub_merged     = types.MethodType(divest_push_to_hub_merged,     model)
        model.save_pretrained_merged = types.MethodType(divest_save_pretrained_merged, model)
        model.push_to_hub_gguf       = types.MethodType(divest_push_to_hub_gguf,       model)
        model.save_pretrained_gguf   = types.MethodType(divest_save_pretrained_gguf,   model)
    pass
    return model
pass