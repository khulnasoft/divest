...
def LlamaModel_fast_forward(
    ...
    *args, **kwargs,
    ...
    if input_ids is not None and inputs_embeds is not None:
        raise ValueError("Divest: You cannot specify both decoder_input_ids and decoder_inputs_embeds at the same time")
    elif input_ids is not None:
        batch_size, seq_length = input_ids.shape
    elif inputs_embeds is not None:
        batch_size, seq_length, _ = inputs_embeds.shape
    else:
        raise ValueError("Divest: You have to specify either decoder_input_ids or decoder_inputs_embeds")
    ...
    if hasattr(self, "max_seq_length"):
        if seq_length > self.max_seq_length:
            logger.warning_once(
                f"Divest: Input IDs of length {seq_length} > the model's max sequence length of {self.max_seq_length}.\n"\
    ...
    if past_key_values is None and self.training:
        ...
        # if use_cache:
        #     logger.warning_once(
        #         "Divest: `use_cache=True` is incompatible with gradient checkpointing. Setting `use_cache=False`"
        #     )
        #     use_cache = False
    pass
    ...
    for idx, decoder_layer in enumerate(self.layers):
        ...
        if offloaded_gradient_checkpointing:
            hidden_states = Divest_Offloaded_Gradient_Checkpointer.apply(
                decoder_layer,
                hidden_states,
                causal_mask,
                attention_mask,
...
class FastLlamaModel:
    ...
    @staticmethod
    def from_pretrained(
        model_name     = "divest/llama-2-7b-bnb-4bit",
        max_seq_length = None,
        dtype          = None,
        load_in_4bit   = True,
        token          = None,
        ...
        **kwargs,
        ...
        statistics = \
           f"==((====))==  Divest: Fast {model_patcher.__name__[4:-5]} patching release {__version__}\n"\
           f"   \\\   /|    GPU: {gpu_stats.name}. Max memory: {max_memory} GB. Platform = {platform_system}.\n"\
           f"O^O/ \_/ \\    Pytorch: {torch.__version__}. CUDA = {gpu_stats.major}.{gpu_stats.minor}. CUDA Toolkit = {torch.version.cuda}.\n"\
           f"\        /    Bfloat16 = {str(SUPPORTS_BFLOAT16).upper()}. Xformers = {xformers_version}. FA = {HAS_FLASH_ATTENTION}.\n"\
           f' "-____-"     Free Apache license: http://github.com/khulnasoft/divest'
        print(statistics)
        ...
        if (rope_scaling is None) and (max_seq_length > model_max_seq_length):
            ...
            logger.warning_once(
                f"Divest: {model_name} can only handle sequence lengths of at most "\
        ...
        try:
            ...
            if "rope_scaling" in str(error):
                if rope_scaling is not None:
                    raise TypeError("Divest: {model_name} does not support rope_scaling.")
                pass
        ...
        f"==((====))==  Divest - 2x faster free finetuning | Num GPUs = {args.world_size}\\n"\\
        ...
        inner_training_loop = inner_training_loop.replace(
            ...
            "raise RuntimeError('Divest: TPUs are not yet supported!')"
        )
        inner_training_loop = inner_training_loop.replace(
            "self.accelerator.free_memory()",
            "self.accelerator.free_memory()\n" + \
            front_spaces + "if self.is_deepspeed_enabled:"\
            "raise RuntimeError('Divest: Deepspeed is not yet supported!')\n", 1,
        )
        ...
        if False:
            ...
            if name.startswith("divest/") and name.endswith("-bnb-4bit"):
                name = name[:len(name) - len("-bnb-4bit")]
                model.config.update({"_name_or_path" : name})
            pass
        pass

        # Log Divest version for future fastpaths for inference
        model.config.update({"divest_version" : __version__})
    ...
    def get_peft_model(
        ...
        loftq_config        = {},
        temporary_location  = "_divest_temporary_saved_buffers",
        **kwargs,
    ):
        ...
        if isinstance(model, PeftModelForCausalLM):
            raise TypeError(
                "Divest: Your model already has LoRA adapters. No need to run this again!"
            )
        pass
        ...
        if lora_dropout != 0:
            logger.warning_once(
                f"Divest: Dropout = 0 is supported for fast patching. You are using dropout = {lora_dropout}.\n"\
                f"Divest will patch all other layers, except LoRA matrices, causing a performance hit."
            )
        pass

        if bias != "none":
            logger.warning_once(
                f"Divest: bias = `none` is supported for fast patching. You are using bias = {bias}.\n"\
                f"Divest will patch all other layers, except LoRA matrices, causing a performance hit."
            )
        pass
        ...
        if not (type(init_lora_weights) is bool or \
            ...
            raise ValueError(
                'Divest: `init_lora_weights` must be either [True, False, "gaussian", "loftq"].'
            )
        pass
        ...
        if init_lora_weights == "loftq":
            ...
            if not SUPPORTS_LOFTQ:
                import peft
                raise RuntimeError(
                    f"Divest: Your PEFT version of {peft.__version__} does not support LoftQ init.\n"\
            ...
            if loftq_config == {}:
                ...
                logger.warning_once(
                    f"Divest: init_lora_weights = `loftq` is set, but `loftq_config` is None.\n"\
            ...
            if hasattr(model.config, "quantization_config"):
                raise ValueError(
                    "Divest: You are using `loftq` init, yet `load_in_4bit = True` was set.\n"\
        ...
        if use_rslora:
            if not SUPPORTS_RSLORA:
                ...
                import peft
                raise RuntimeError(
                    f"Divest: Your PEFT version of {peft.__version__} does not support `use_rslora`.\n"\
                    "Please install PEFT 0.7.2 or higher.\n"\
                    "You can also install from source: `pip install git+https://github.com/huggingface/peft.git"
                )
            pass
        ...
        for module in target_modules:
            if module == "lm_head":
                # logger.warning_once(
                #     "Divest: `lm_head` should be placed in `modules_to_save` and not `target_modules`. "\
                ...
                # logger.warning_once(
                #     "Divest: `embed_tokens` should be placed in `modules_to_save` and not `target_modules`. "\
                #     "Luckily, we shall do it for you!"
                # )
        ...
        if hasattr(model, "_need_to_train_embeddings"):
            if not train_lm_head or not train_embed_tokens:
                print(
                    "Divest: You added new tokens but did not specify if you wanted to "\
        ...
        if modules_to_save is not None:
            for module in modules_to_save:
                if module == "lm_head":
                    ...
                    raise TypeError(
                        f"Divest: Module = {module} is not allowed. Only 'lm_head' and 'embed_tokens' is allowed."
                    )
            pass
        pass
        ...
        if use_gradient_checkpointing == "divest":
            if train_embed_tokens:
                print("Divest: Offloading input_embeddings to disk to save VRAM")
            ...
            if train_lm_head:
                print("Divest: Offloading output_embeddings to disk to save VRAM")
        ...
        if train_embed_tokens:
            print("Divest: Casting embed_tokens to float32")
        ...
        if train_lm_head:
            print("Divest: Casting lm_head to float32")
    ...
    def patch_peft_model(
        ...
        if not isinstance(model, PeftModelForCausalLM):
            raise TypeError(
                "Divest: Your model needs to call `.get_peft_model` first!"
            )
        pass
        ...
        if   model_type == "llama":   apply_lora_mlp = apply_lora_mlp_swiglu
            ...
            raise NotImplementedError(f"Divest: {model_type} is not yet implemented!")
        pass
        ...
        for active_adapter in model.peft_config.keys():
            ...
            model.peft_config[active_adapter].revision = f"divest"
        pass
        ...
        if lora_dropout == 0 and bias == "none":
            for idx, layer in enumerate(model.model.model.layers):
                ...
                if  hasattr(gate_proj, "lora_A") and \
                    ...
                    logger.warning_once(
                        "Not an error, but Divest cannot patch MLP layers with our manual autograd engine since either LoRA adapters\n"\
                ...
                if  hasattr(q_proj, "lora_A") and \
                    ...
                    if model_type != "qwen2":
                        logger.warning_once(
                            "Not an error, but Divest cannot patch Attention layers with our manual autograd engine since either LoRA adapters\n"\
                            "are not enabled or a bias term (like in Qwen) is used."
                        )
                    pass
                pass
                ...
                if hasattr(o_proj, "lora_A") and \
                    ...
                    logger.warning_once(
                        "Not an error, but Divest cannot patch O projection layer with our manual autograd engine since either LoRA adapters\n"\
                        "are not enabled or a bias term (like in Qwen) is used."
                    )
                pass
            pass
        ...
        logger.warning_once(
            f"Divest {__version__} patched {len(model.model.model.layers)} layers with "\
