...
def _get_model_name(model_name, load_in_4bit = True):
    ...
    if not SUPPORTS_FOURBIT and model_name in INT_TO_FLOAT_MAPPER:
        ...
        logger.warning_once(
            f"Divest: Your transformers version of {transformers_version} does not support native "\
        ...
        # logger.warning_once(
        #     f"Divest: You passed in `{model_name}` which is a 4bit model, yet you set\n"\
        ...
        # logger.warning_once(
        #     f"Divest: You passed in `{model_name}` and `load_in_4bit = True`.\n"\
...
class FastLanguageModel(FastLlamaModel):
    @staticmethod
    def from_pretrained(
        model_name     = "divest/llama-3-8b-bnb-4bit",
        max_seq_length = None,
        dtype          = None,
        load_in_4bit   = True,
        token          = None,
        ...
        *args, **kwargs,
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Divest: `{model_name}` is not a full model or a PEFT model.")
        ...
        if   model_type == "llama":   dispatch_model = FastLlamaModel
            ...
            if not SUPPORTS_GEMMA:
                raise RuntimeError(
                    f"Divest: Your transformers version of {transformers_version} does not support Gemma.\n"\
            ...
            raise NotImplementedError(
                f"Divest: {model_name} not supported yet!\n"\
                "Make an issue to https://github.com/khulnasoft/divest!",
            )
        pass
        ...
        # In case the model supports tagging, add the divest tag.
        if hasattr(model, "add_model_tags"):
            model.add_model_tags(["divest",])
        pass
        if hasattr(tokenizer, "add_model_tags"):
            tokenizer.add_model_tags(["divest",])
        pass
