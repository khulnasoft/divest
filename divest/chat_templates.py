...
# Divest efficient template leverages from Zephyr
divest_template = \
    "{{ bos_token }}"\
    ...
    "{% if add_generation_prompt %}"\
        "{{ '>>> Assistant: ' }}"\
    "{% endif %}"
divest_eos_token = "eos_token"
CHAT_TEMPLATES["divest"] = (divest_template, divest_eos_token, False,)
...
def get_chat_template(
    ...
    if type(chat_template) in (list, tuple,):
        ...
        if not is_fast_tokenizer:
            print(
                f"Divest: Not a fast tokenizer, so can't process it as of yet :(\n"\
            ...
            if map_eos_token and (not stop_word in token_mapping.values()):
                ...
                logger.warning_once(f"Divest: Will map {stop_word} to EOS = {tokenizer.eos_token}.")
        ...
        raise TypeError(
            f"Divest: `chat_template` must be a tuple of (your_template, eos_token,) or one of\n"\
...
def test_chat_templates():
    ...
    correct_tokenizer = AutoTokenizer.from_pretrained("divest/llama-2-7b-chat")
    correct_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    correct_tokenizer.chat_template = template
    our_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    assert(correct_prompt == our_prompt)
    ...
    # Gemma
    correct_tokenizer = AutoTokenizer.from_pretrained("divest/gemma-7b-it")
    correct_prompt = correct_tokenizer.apply_chat_template(messages[1:], tokenize = False, add_generation_prompt = True)
    ...
    correct_tokenizer = AutoTokenizer.from_pretrained("divest/llama-3-8b-Instruct")
    correct_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    correct_tokenizer.chat_template = template
    our_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    assert(correct_prompt == our_prompt)
...
def test_hf_gguf_equivalence(tokenizer, gguf_model = "./model-divest.F16.gguf"):
    """
