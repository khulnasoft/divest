...
if "CUDA_VISIBLE_DEVICES" in os.environ:
    ...
    if not devices.isdigit():
        ...
        warnings.warn(
            f"Divest: 'CUDA_VISIBLE_DEVICES' is currently {devices} \n"\
            "Multiple CUDA devices detected but we require a single device.\n"\
            f"We will override CUDA_VISIBLE_DEVICES to first device: {first_id}."
        )
        os.environ["CUDA_VISIBLE_DEVICES"] = str(first_id)
else:
    # warnings.warn("Divest: 'CUDA_VISIBLE_DEVICES' is not set. We shall set it ourselves.")
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
...
if (major_torch < 2):
    raise ImportError("Divest only supports Pytorch 2 for now. Please update your Pytorch to 2.1.\n"\
                      "We have some installation instructions on our Github page.")
...
try:
    ...
    warnings.warn(
        "Divest: Running `ldconfig /usr/lib64-nvidia` to link CUDA."\
    )
    ...
    try:
        ...
        warnings.warn(
            "Divest: CUDA is not linked properly.\n"\
            "Try running `python -m bitsandbytes` then `python -m xformers.info`\n"\
            "We tried running `ldconfig /usr/lib64-nvidia` ourselves, but it didn't work.\n"\
            "You need to run in your terminal `sudo ldconfig /usr/lib64-nvidia` yourself, then import Divest.\n"\
            "Also try `sudo ldconfig /usr/local/cuda-xx.x` - find the latest cuda version.\n"\
            "Divest will still run for now, but maybe it might crash - let's hope it works!"
        )
pass
