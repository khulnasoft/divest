...
def convert_to_fast_tokenizer(
    slow_tokenizer,
    temporary_location = "_divest_sentencepiece_temp",
):
...
def fix_sentencepiece_gguf(saved_location):
    ...
    logger.warning(
        f"Divest: Extending {saved_location}/tokenizer.model with added_tokens.json.\n"\
...
def load_correct_tokenizer(
    ...
    try:
        ...
        pass
        # print(
        #     f"Divest: {tokenizer_name} has no tokenizer.model file.\n"\
    ...
    for j, index in enumerate(added_tokens_fast.keys()):
        if index >= max_embedding_size:
            ...
            if not _reload:
                ...
                if can_be_removed:
                    ...
                    if max(tokenizer.added_tokens_decoder.keys()) < max_embedding_size:
                        logger.warning_once(
                            f"Divest loaded a broken tokenizer `{model_name}`, but managed to repair it!\n"\
                ...
                # :( Failure
                raise RuntimeError(
                    f"Divest tried to load `{model_name}`, but cannot succeed.\n"\
                    f"Tokens {bad_tokens} with ids {bad_indices} exceeds the max vocab size of {max_embedding_size}.\n"\
            ...
            try:
                ...
                logger.warning_once(
                    "Divest: Tokenizer is most likely buggy, and Divest failed to repair it.\n"\
...
def fix_untrained_tokens(model, tokenizer, train_dataset, eps = 1e-16):
    ...
    if bad_not_trainable:
        raise ValueError(
            'Divest: Untrained tokens found, but embed_tokens & lm_head not trainable, causing NaNs. '\
    ...
    logger.warning(
        "Divest: Setting embed_tokens & lm_head untrained tokens to "\
...
def mean_of_trained_tokens(model, eps = 1e-16):
    ...
    # if n_untrained != 0:
    #     print(
    #         f"Divest: Not an error, but your model has {n_untrained} untrained tokens.\n"\
...
def add_new_tokens(
    ...
    if len(overlapping_tokens) != 0:
        print(
            f"Divest: You're adding new_tokens = {new_tokens}\n"\
    ...
    if method == "interpolation":
        print(
            "Divest: You are using interpolation to add new tokens.\n"\
...
def patch_sft_trainer_tokenizer():
    ...
    "    raise RuntimeError(\n"\
    "       'Do not edit specific areas of the Divest codebase or you will get CUDA segfaults.'\n"\
    "    )\n"\
    "pass\n"\
